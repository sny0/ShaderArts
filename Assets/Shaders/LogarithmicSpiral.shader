Shader "Unlit/LogarithmicSpiral"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float LogarithmicSpiral(float r, float theta, float a, float b) {
                float spiral[ArrayCount];

                for (int i = 0; i < ArrayCount; i++) {
                    spiral[i] = a * exp( b * (theta + 2 * PI * i));
                }

                float d = 0;
                for (int i = 0; i < ArrayCount; i++) {
                    d += step(spiral[i] - (theta + 2 * PI * i) * 0.001, r) * step(r, spiral[i]);
                }
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float2 prePolarP1 = float2(1, -1 * _Time.y * 5);

                i.uv = Rotate(prePolarP0, prePolarP1);
                float theta = atan2(i.uv.y, i.uv.x) + PI;
                float l = length(i.uv);

                //float d = LogarithmicSpiral(l, theta, 0.01, 0.16);
                float r = LogarithmicSpiral(l, theta, 0.01, 0.16);
                float g = LogarithmicSpiral(l, theta, 0.02, 0.17);
                float b = LogarithmicSpiral(l, theta, 0.05, 0.18);
