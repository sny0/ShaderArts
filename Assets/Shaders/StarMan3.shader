        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float dy= sin(2 * i.uv.x- _Time.y * 5);
                i.uv.x += 0.25 * dx;
                i.uv.y += 0.25 * dy;
                ...
                float threshold = 0.3 + 0.8 * sin(random(float2(masux, masuy)));
                
                float d = step(length(i.uv), threshold);

                float h = (masux * 1.0) / 50 + _Time.y * 1;
                float s = (masuy * 1.0) / 30 + _Time.y * 5;
                float l = (masuy * 1.0) / 10 + 1 * sin(_Time.y * 3);
                h = frac(h);
                s = frac(s);
                l = frac(l) * 0.85;
